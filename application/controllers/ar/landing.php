...
class Landing  extends CI_Controller 
{
	...
	public function login()
		...
		if($email){
			...
			if($result)
			{
			   ...
			   $ret_arr['success']  =false;
				...
				if( $this->usermodel->add_user($activation)==0){
	 ...
	 {
			if($this->session->userdata('language')=='englisg')
				$data['postfix']='';
			else
				$data['postfix']='_ar';	
			...
			if( $this->input->post('update')){
				...
				//if($this->input->post('updid')){
					...
					if($this->usermodel->update_user($udata)){
					    redirect("index.php/ar/landing/my_account/updsuccess");
					}else{
						redirect("index.php/ar/landing/my_account/upderror");
					}
